...
C_sh_G value of gas lost load [$ per kNm^3] /100/
...
Gas_bal(t).. sum(k, g_DA(k,t)) - sum(g, idata(g,'phi')*p_DA(g,t)) - sum(ssg, idata(ssg,'phi')*p_DA(ssg,t))  + sum(q, v_DA_G(q,t)) - D_G(t) =e= 0;
...
RT_Gas_balance(t,omega).. sum(k, g_RT(k,t,omega)) - sum(g, idata(g,'phi')*p_RT(g,t,omega)) - sum(ssg, idata(ssg,'phi')*p_RT(ssg,t,omega)) + l_sh_G(t,omega) + sum(q, v_RT_G(q,t)) =e= 0;
